...
from ..utils import koalby_time as time
...
class Primitive(StoppableThread):
    ...
    def start(self):
        """ Start or restart (the :meth:`~pypot.primitive.primitive.Primitive.stop` method
        will automatically be called) the primitive. """
    ...
    def affect_once(self, motor, register, value):
        with self.being_synced:
            setattr(motor, register, value)  # change from register to set commands
...
class LoopPrimitive(Primitive):
    ...
    def run(self):
        """ Calls the :meth:`~pypot.utils.stoppablethread.StoppableLoopThread.update` method
        at a predefined frequency (runs until stopped). """
...
#     """ Fake :class:`~pypot.robot.robot.Robot` used by the :class:
#     `~pypot.primitive.primitive.Primitive` to ensure sandboxing. """
#     def __init__(self, robot):
#         self._robot = robot
#         self._motors = []
#
